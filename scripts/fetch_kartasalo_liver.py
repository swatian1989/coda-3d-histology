#!/usr/bin/env python
"""Stream the Kartasalo supplementary ZIP and extract only the liver stack.

    python scripts/fetch_kartasalo_liver.py [--recon] [--keep PATTERN]
...
KEEP_DEFAULT = r"liver|landmark|fiducial|hole"
...
def main() -> None:
    ...
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(message)s",
                        handlers=[logging.FileHandler(ROOT / "logs/kartasalo_fetch.log"),
    ...
    url = authorize()
    logger.info("authorized; streaming 63.79 GB (service has no range support, "
                "so this cannot resume)")
    t0 = time.time()
    n_seen = n_kept = 0
    kept_bytes = 0
